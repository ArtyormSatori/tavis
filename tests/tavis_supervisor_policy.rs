...
#[test]
fn supervisor_treats_omniroute_failure_as_critical_dependency() {
    let mut components = BTreeMap::new();
    components.insert("core".into(), component("ok", 0));
    components.insert("omniroute".into(), component("error", 1));
    let snapshot = HealthSnapshot {
        pid: 1,
        updated_at: "now".into(),
        uptime_seconds: 10,
    ...
    assert_eq!(
        supervisor_decision(&snapshot, policy),
        SupervisorDecision::RestartAfter { seconds: 4 }
    );
}
