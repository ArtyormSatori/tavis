...
//! any engine or host type. They are separated from [`crate::openhuman::memory::api::types`] because
//! that module carries the historical engine value types (which the engine
//! crate aliases back into `crate::openhuman::memory::engine::types`), whereas these are new
...
/// One unit of content handed to [`crate::openhuman::memory::api::provider::MemoryIngest`].
///
...
///
/// Paging (rather than a stream) keeps [`crate::openhuman::memory::api::provider::MemoryPortability`]
...
/// One item handed to [`crate::openhuman::memory::api::provider::MemorySourceSink`] by the host's sync
/// machinery.
///
