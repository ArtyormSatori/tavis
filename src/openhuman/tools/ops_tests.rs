...
/// (d) Tools degrade through the SINGLE chokepoint, exhaustively and
/// non-vacuously: every registered tool whose `tool_capability` is an OPTIONAL
/// family disappears under the null driver, and every tool whose capability is
/// `None` or a MANDATORY family survives. Derived from the live tool list and
/// the live `tool_capability` fn — no hand-written table to drift.
#[tokio::test]
async fn zzverify_exhaustive_tool_degradation_under_null_driver() {
    use crate::core::runtime::context::CoreContext;
    use crate::core::runtime::DomainSet;
    use tinycortex_api::capabilities::{Capabilities, Capability};

    let tmp = TempDir::new().unwrap();
    let baseline = tool_names(&expansion_tools_for(&tmp));
    assert!(baseline.len() > 50, "sanity: {} tools", baseline.len());

    let mandatory = Capabilities::mandatory();
    let mut should_vanish = Vec::new();
    let mut should_survive = Vec::new();
    for n in &baseline {
        match super::tool_capability(n) {
            Some(c) if !mandatory.contains(c) => should_vanish.push(n.clone()),
            _ => should_survive.push(n.clone()),
        }
    }
    assert!(
        !should_vanish.is_empty(),
        "NON-VACUITY: no optional-family tools are registered at all"
    );
    // Independent spot-check that the mandatory set is what we think it is.
    assert!(mandatory.contains(Capability::Core) && mandatory.contains(Capability::Recall));
    assert!(!mandatory.contains(Capability::Tree) && !mandatory.contains(Capability::Goals));

    let tmp2 = TempDir::new().unwrap();
    let ctx = CoreContext::for_test(
        DomainSet::full(),
        Some(std::path::PathBuf::from("/tmp/oh-zzverify-tools-null")),
        Some(crate::openhuman::config::schema::MemorySubsystemConfig {
            driver: "null".into(),
            ..Default::default()
        }),
    );
    let under_null =
        CoreContext::scope(ctx, async { tool_names(&expansion_tools_for(&tmp2)) }).await;

    let leaked: Vec<&String> = should_vanish
        .iter()
        .filter(|n| under_null.contains(n))
        .collect();
    let over_gated: Vec<&String> = should_survive
        .iter()
        .filter(|n| !under_null.contains(n))
        .collect();

    assert!(leaked.is_empty(), "optional-family tools survived the null driver: {leaked:?}");
    assert!(
        over_gated.is_empty(),
        "OVER-GATING: mandatory/ungated tools vanished under the null driver: {over_gated:?}"
    );
    // The mandatory memory surface specifically.
    for must in [
        "memory_store",
        "memory_recall",
        "memory_forget",
        ...
        assert!(
            under_null.iter().any(|n| n == must),
            "MANDATORY tool `{must}` must survive every configuration"
        );
    }
    eprintln!(
        "[zzverify] tools: {} baseline -> {} under null ({} expected to vanish)",
        baseline.len(),
        under_null.len(),
        should_vanish.len()
    );
...
/// (a) DEFAULT OPEN for tools, exhaustively: with NO ambient context the
/// capability filter removes nothing — every tool that `tool_capability` maps
/// to an optional family is still in the list.
#[test]
fn zzverify_default_open_tools_lose_nothing() {
    let tmp = TempDir::new().unwrap();
    let names = tool_names(&expansion_tools_for(&tmp));
    let optional: Vec<&String> = names
        .iter()
        .filter(|n| {
            super::tool_capability(n)
                .is_some_and(|c| !tinycortex_api::capabilities::Capabilities::mandatory().contains(c))
        })
        .collect();
    assert!(!optional.is_empty(), "NON-VACUITY: nothing optional in the list");
    eprintln!("[zzverify] default-open keeps {} optional-family tools", optional.len());
}
