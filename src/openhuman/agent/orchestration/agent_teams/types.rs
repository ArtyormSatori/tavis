...
//! [`ClaimOutcome`]) live in `tinyagents::harness::session_store::run_ledger`. This module adds the
//! read-aggregate view returned by the controllers and the validation error
//! surface used by `ops::assign_task`.

use serde::Serialize;

use tinyagents::harness::session_store::run_ledger::{AgentTeam, AgentTeamMember, AgentTeamTask};
