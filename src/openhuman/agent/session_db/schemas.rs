...
use tinyagents::harness::session_store::run_ledger::{AgentRunListRequest, RunEventListRequest};
use tinyagents::harness::session_store::types::SessionSearchParams;
...
fn handle_session_db_list(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let result = tinyagents::harness::session_store::list_sessions(
...
fn handle_session_db_get(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let session = tinyagents::harness::session_store::get_session(&config.workspace_dir, id).map_err(|e| {
            let s = e.to_string();
...
fn handle_session_db_search(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let result = tinyagents::harness::session_store::search_sessions(
            &config.workspace_dir,
            &search_params,
        )
        .map_err(|e| {
...
fn handle_session_db_get_messages(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let messages = tinyagents::harness::session_store::list_messages(&config.workspace_dir, session_id, limit).map_err(|e| {
            let s = e.to_string();
...
fn handle_session_db_get_tool_calls(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let tool_calls = tinyagents::harness::session_store::list_tool_calls(&config.workspace_dir, session_id, limit).map_err(|e| {
            let s = e.to_string();
...
fn handle_session_db_get_children(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let children = tinyagents::harness::session_store::list_children(&config.workspace_dir, session_id).map_err(|e| {
            let s = e.to_string();
...
fn handle_run_ledger_list(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let response = tinyagents::harness::session_store::run_ledger::list_agent_runs(
            &config.workspace_dir,
            &request,
        )
        .map_err(|e| {
...
fn handle_run_ledger_get(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let id = params
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| "missing required param: id".to_string())?;
        let run = tinyagents::harness::session_store::run_ledger::get_agent_run(&config.workspace_dir, id).map_err(|e| {
            let s = e.to_string();
...
fn handle_run_ledger_events(params: Map<String, Value>) -> ControllerFuture {
    Box::pin(async move {
        ...
        let response = tinyagents::harness::session_store::run_ledger::list_recent_run_events(
            &config.workspace_dir,
            &request,
        )
        .map_err(|e| {
