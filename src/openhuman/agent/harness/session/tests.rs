...
// ─── ADVERSARIAL VERIFICATION (temporary) ────────────────────────────

/// Drive the REAL default (file) locator path against a transcript that has a
...
async fn adversarial_compacted_transcript_replays_through_both_reads() {
    ...
    // Turn 1: the pre-compaction history.
    let turn1 = vec![
        ChatMessage::system("system prompt"),
        ChatMessage::user("PRECOMPACT_U1"),
        ChatMessage::assistant("PRECOMPACT_A1"),
        ChatMessage::user("PRECOMPACT_U2"),
        ChatMessage::assistant("PRECOMPACT_A2"),
    ];
    transcript::append_transcript_turn(&path, &[], &turn1, &meta, None, None).expect("turn 1");

    // Turn 2: a genuine context REDUCTION -> compaction record.
    let turn2 = vec![
        ChatMessage::system("system prompt"),
        ChatMessage::assistant("SUMMARY_OF_EARLIER"),
        ChatMessage::user("POSTCOMPACT_U3"),
        ChatMessage::assistant("POSTCOMPACT_A3"),
    ];
    transcript::append_transcript_turn(&path, &turn1, &turn2, &meta, None, None).expect("turn 2");

    // Then a cancelled stream leaves an interrupted partial on disk.
    transcript::append_interrupted_partial(&path, "TRUNCATED_PARTIAL", None, Some(1), None)
        .expect("interrupted partial");

    let raw = std::fs::read_to_string(&path).unwrap();
    assert!(
        raw.contains("\"kind\":\"compaction\""),
        "fixture must actually contain a compaction record:\n{raw}"
    );
    assert!(
        raw.contains("PRECOMPACT_U1"),
        "pre-compaction lines must still be on disk"
    );
    assert!(
        raw.contains("\"interrupted\":true"),
        "fixture must actually contain an interrupted partial:\n{raw}"
    );
    ...
    let mk = |name: &str| {
        let mem: Arc<dyn Memory> = Arc::from(
            crate::openhuman::memory::store::create_memory(&memory_cfg, &wsp).unwrap(),
        );
        Agent::builder()
            .chat_model(Arc::new(MockProvider {
                responses: Mutex::new(vec![]),
            ...
            .memory(mem)
            .tool_dispatcher(Box::new(NativeToolDispatcher))
            .agent_definition_name(name)
            .workspace_dir(wsp.clone())
            .build()
            .expect("agent build should succeed")
    };

    // (1) stem-keyed resume (try_load_session_transcript)
    let mut a1 = mk("orchestrator");
    a1.try_load_session_transcript();
    let cached = a1
        .cached_transcript_messages
        .as_ref()
        .expect("stem-keyed resume must load the transcript");
    assert_eq!(
        cached.iter().map(|m| m.content.as_str()).collect::<Vec<_>>(),
        expected,
        "stem-keyed resume context must be the post-compaction set only"
    );

    // (2) thread-keyed cold-boot resume
    let mut a2 = mk("some_other_agent_name");
    assert!(
        a2.seed_resume_from_thread_transcript(thread_id),
        "thread-keyed resume must load the root transcript"
    );
    let cached2 = a2
        .cached_transcript_messages
        .as_ref()
        .expect("thread-keyed resume must load the transcript");
    assert_eq!(
        cached2.iter().map(|m| m.content.as_str()).collect::<Vec<_>>(),
        expected,
        "thread-keyed resume context must be the post-compaction set only"
    );

    // (3) The display projection still sees everything (nothing was destroyed).
    let display = transcript::read_transcript_display(&path).expect("display read");
    let dump = format!("{display:?}");
    assert!(
        dump.contains("PRECOMPACT_U1") && dump.contains("TRUNCATED_PARTIAL"),
        "display projection must retain pre-compaction history and the partial"
    );
}
