...
fn btc_derives_an_address_its_own_sender_rule_accepts() {
    ...
    let key = derive(Chain::Btc, VECTOR, BTC_PATH).unwrap();
    assert!(
        crate::openhuman::web3::wallet::primitives::address::btc::validate_sender(key.address())
            .is_ok()
    );
}
...
fn every_chain_derives_an_address_its_own_validator_accepts() {
    ...
    for (chain, path) in [
        ...
        assert!(
            crate::openhuman::web3::wallet::primitives::address::validate(chain, key.address())
                .is_ok(),
...
fn tron_derives_a_mainnet_address_not_an_evm_one() {
    ...
    let decoded =
        crate::openhuman::web3::wallet::primitives::address::tron::decode(key.address()).unwrap();
    assert_eq!(decoded.len(), 21);
    assert_eq!(
        decoded[0],
        crate::openhuman::web3::wallet::primitives::address::tron::MAINNET_PREFIX
    );
}
