...
/// All node-kind contracts with this host's overlay applied, in
/// [`NODE_KINDS`] order.
...
/// The overlaid contract for one node kind, or `None` if `kind` is unknown.
...
mod tests {
    ...
    #[test]
    fn overlay_preserves_all_vendor_kinds() {
