...
| `src/openhuman/security/keyring/store.rs` | Backend selection + global state. Owns `WORKSPACE_DIR` and `BACKEND` `OnceLock`s, `init_workspace`, `backend()`, `build_backend()`, and `workspace_dir_for_file_backend()` path derivation. |
