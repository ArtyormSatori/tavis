...
| `REGISTRY_SOFT_CAP` (256) | **HOST-OWNED — no action** | Already crate-owned as *policy*: it is passed into `DetachedTaskRegistry::new`, which performs the sweep. Only the number is OpenHuman's. Recorded because an earlier scoping pass mistook it for a host-side duplicate. |
